...
import com.example.core.presentation.utils.toPresentationMessage
import io.ktor.http.*
...
fun StatusPagesConfig.configure400Responses(
    ...
    exception<InvalidIdTokenException> { call, cause ->
        logger.error("[Id token validation] - ${cause.message}: ${cause.error.toPresentationMessage()}")
    ...
    exception<InvalidSessionException> { call, cause ->
        logger.error("[Session validation] - ${cause.message}: ${cause.error.toPresentationMessage()}")
    ...
    exception<InvalidUserException> { call, cause ->
        logger.error("[User validation] - ${cause.message}: ${cause.error.toPresentationMessage()}")
